        ...
        Array_gpu<TF,3>& lw_bnd_flux_up, Array_gpu<TF,3>& lw_bnd_flux_dn, Array_gpu<TF,3>& lw_bnd_flux_net) const
{
            ...
            Fluxes_broadband_gpu<TF>& bnd_fluxes)
    {
        ...
        p_lay.subset(p_lay_subset_in, {{ {col_s_in, col_e_in}, {1, n_lay} }});
        p_lev.subset(p_lev_subset_in, {{ {col_s_in, col_e_in}, {1, n_lev} }});
        t_lay.subset(t_lay_subset_in, {{ {col_s_in, col_e_in}, {1, n_lay} }});
        t_lev.subset(t_lev_subset_in, {{ {col_s_in, col_e_in}, {1, n_lev} }});
        t_sfc.subset(t_sfc_subset_in, {{ {col_s_in, col_e_in} }});

        Array_gpu<TF,2> col_dry_subset({n_col_in, n_lay});
        if (col_dry.size() == 0)
            Gas_optics_rrtmgp_gpu<TF>::get_col_dry(col_dry_subset, gas_concs_subset.get_vmr("h2o"), p_lev_subset_in);
        else
    ...
    Array_gpu<TF,1> t_sfc_subset({n_col_block});
    ...
    if (n_col_block_residual > 0)
    {
        ...
        Array_gpu<TF,1> t_sfc_residual({n_col_block_residual});
